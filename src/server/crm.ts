...
export async function getPublicPts() {
  ...
  const ptServices = await db
    ...
    .where(and(eq(schema.ptServiceOfferings.active, 1), eq(schema.serviceOfferings.active, 1)));
...
export async function getPublicLandingContent() {
  const [branches, plans, services, promotions, events, pts] = await Promise.all([
    getPublicBranches(),
    getPublicPlans(),
    getPublicServices(),
    getPublicPromotions(),
