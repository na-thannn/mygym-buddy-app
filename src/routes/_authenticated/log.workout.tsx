...
function WorkoutLog() {
  const list = async (options?: { limit?: number }) => {
  ...
  const create = async (payload: { data: any }) => {
    await fetch('/api/log/workout', { method: 'POST', credentials: 'include', headers: { 'content-type': 'application/json' }, body: JSON.stringify(payload.data) });
  };
    ...
    <div className="max-w-2xl mx-auto p-4 md:p-6">
      ...
      <form onSubmit={submit} className="rounded-2xl border border-white/10 bg-black/40 backdrop-blur p-4 mb-6 space-y-3 animate-fade-up">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="space-y-1"><Label>Load (kg)</Label><Input type="number" step="0.5" value={f.weightKg} onChange={(e) => setF({ ...f, weightKg: e.target.value })} /></div>
        </div>
        <div className="space-y-1"><Label>Notes</Label><Textarea rows={2} value={f.notes} onChange={(e) => setF({ ...f, notes: e.target.value })} /></div>
        <Button type="submit" className="bg-yellow-400 text-yellow-950 hover:bg-yellow-300">Save entry</Button>
      </form>

      <div className="space-y-2">
        {rows.length === 0 && <div className="text-sm text-slate-400 text-center py-10">No workouts logged yet.</div>}
        {rows.map((r) => (
          <div key={r.id} className="rounded-2xl border border-white/10 bg-black/40 backdrop-blur p-4 transition-all duration-300 hover:-translate-y-0.5 hover:bg-white/5">
