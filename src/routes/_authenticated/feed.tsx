...
import { Heart, MessageSquare, Share2, MoreHorizontal, User, Image as ImageIcon, Loader2 } from "lucide-react";
...
function Feed() {
  ...
  const handlePost = async (imageBase64?: string | null) => {
    ...
    setBusy(true);
    try {
      await fetch('/api/feed', { method: 'POST', credentials: 'include', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ content: newPost.trim() || "", imageBase64: imageBase64 ?? null }) });
      setNewPost("");
      await load();
  ...
  return (
    <div className="mx-auto max-w-2xl p-4 md:p-6 pb-24 md:pb-6">
      <PageHeader title="Community Feed" description="See what's happening at HL Fitness 303 Le Thanh Nghi" />
      ...
      <div className="mt-6 rounded-2xl border border-white/10 bg-black/40 backdrop-blur-xl p-4 shadow-xl">
        <div className="flex gap-4">
          ...
          <div className="flex-1">
            <textarea 
              ...
              className="w-full bg-transparent border-none rounded-lg text-sm text-slate-200 focus:outline-none min-h-[60px] resize-none"
            />
          </div>
        </div>
        <div className="flex justify-between items-center mt-3 pt-3 border-t border-white/5">
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="sm" className="text-slate-400 hover:text-yellow-400 hover:bg-yellow-400/10" onClick={() => fileRef.current?.click()}>
      ...
      <div className="mt-8 space-y-6">
        {posts.map((post) => (
          <div key={post.id} className="rounded-2xl border border-white/10 bg-black/30 p-5 animate-fade-up">
            ...
            {post.imageBase64 && (
              <div className="mt-3 rounded-md overflow-hidden">
                <img src={post.imageBase64} alt="upload" className="w-full h-auto object-cover rounded-md" />
              </div>
            )}
            <div className="mt-5 flex gap-1 border-t border-white/5 pt-3">
              ...
              <Button variant="ghost" size="sm" className="flex-1 text-slate-400 hover:text-slate-200 hover:bg-white/5">
