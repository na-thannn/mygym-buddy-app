...
function Progress() {
  ...
  const fetchPhotos = async () => {
    ...
    return (rows as { imageBase64: string }[]).map((r) => r.imageBase64);
  };
    ...
    <div className="mx-auto max-w-4xl p-4 md:p-8 pb-24 md:pb-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <Button
          ...
          disabled={busy}
          className="bg-yellow-400 text-yellow-950 hover:bg-yellow-300 gap-2 mb-2 md:mb-0 w-full md:w-auto"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-fade-in">
        {photos.map((src, idx) => (
          <div
            key={idx}
            className="aspect-[3/4] rounded-xl overflow-hidden border border-white/10 group relative"
          >
            <img
              src={src}
              alt="Progress"
        ...
        {photos.length < 4 && (
          <div
            onClick={handleUploadClick}
            className="aspect-[3/4] rounded-xl border border-white/10 bg-white/5 flex flex-col items-center justify-center text-slate-500 hover:bg-white/10 hover:text-slate-300 transition cursor-pointer"
          >
      ...
      <div className="mt-10 rounded-2xl border border-white/10 bg-black/40 backdrop-blur p-6 animate-fade-in flex items-center justify-between group cursor-pointer hover:bg-white/5 transition">
        <div>
          ...
          <p className="text-slate-400 text-sm max-w-sm">
            Get personalized insights on your progress from Alex.
          </p>
        </div>
