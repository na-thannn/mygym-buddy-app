    ...
    <div className="max-w-2xl mx-auto p-4 md:p-6">
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1">
            <Label>Lunch</Label>
            <Input value={f.lunch} onChange={(e) => setF({ ...f, lunch: e.target.value })} />
          </div>
          <div className="space-y-1">
            <Label>Dinner</Label>
            <Input value={f.dinner} onChange={(e) => setF({ ...f, dinner: e.target.value })} />
          </div>
          <div className="space-y-1">
            <Label>Snacks</Label>
            <Input value={f.snacks} onChange={(e) => setF({ ...f, snacks: e.target.value })} />
          </div>
          <div className="space-y-1">
      ...
      <div className="space-y-2">
        {rows.length === 0 && (
          <div className="text-sm text-slate-400 text-center py-10">No reports yet.</div>
        )}
        {rows.map((r) => (
          <div
            key={r.id}
            className="rounded-2xl border border-white/10 bg-black/40 backdrop-blur p-4 text-sm transition-all duration-300 hover:-translate-y-0.5 hover:bg-white/5"
          >
            <div className="font-medium text-slate-100">
              {formatDate(r.reportDate)}{" "}
              <span className="text-xs text-slate-400 font-normal">• {r.dayType ?? "—"}</span>
            </div>
            <div className="text-xs text-slate-400 mt-0.5">
              {r.calories ?? "—"} kcal • P {r.proteinG ?? 0}g • C {r.carbsG ?? 0}g • F{" "}
              {r.fatsG ?? 0}g
            </div>
