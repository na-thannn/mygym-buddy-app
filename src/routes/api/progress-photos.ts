  ...
  server: {
    handlers: {
      ...
      POST: async (request: Request) => {
        ...
        if (!body || !body.imageBase64) return new Response(JSON.stringify({ error: 'Missing imageBase64' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        const id = newId();
        try {
          db.insert(schema.progressPhotos).values({ id, userId: session.userId, imageBase64: body.imageBase64 }).run();
          return new Response(JSON.stringify({ ok: true, id }), { status: 200, headers: { 'Content-Type': 'application/json' } });
        } catch (err: any) {
          await logDevError({ error: err, req: { method: 'POST', url: '/api/progress-photos', body } }).catch(() => {});
