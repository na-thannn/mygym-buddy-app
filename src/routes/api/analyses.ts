...
export const Route = createFileRoute('/api/analyses')({
  server: {
    handlers: {
      GET: async () => {
      ...
      POST: async (request: any) => {
        ...
        const id = newId();
        try {
          db.insert(schema.analyses).values({ id, userId: session.userId, planDate: body.planDate, contentMd: body.contentMd }).run();
          return new Response(JSON.stringify({ ok: true, id }), { status: 200, headers: { 'Content-Type': 'application/json' } });
        } catch (err: any) {
          await logDevError({ error: err, req: { method: 'POST', url: '/api/analyses', body } }).catch(() => {});
