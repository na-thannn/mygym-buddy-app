(async () => {
  ...
  try {
    // Signup
    let rObj = await doFetch("/api/signup", {
      method: "POST",
    ...
    if (!setCookie || !setCookie.includes("alex_session")) {
      const sObj = await doFetch("/api/signin", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    const tests = [
      {
        name: "/api/log/nutrition-report",
        path: "/api/log/nutrition-report",
        body: { reportDate: now, calories: 2000 },
      },
      {
        name: "/api/inbody",
        path: "/api/inbody",
        body: { reportDate: now, weightKg: 72, muscleMassKg: 30, bodyFatPercent: 15 },
      },
      ...
      { name: "/api/feed", path: "/api/feed", body: { content: "Hello from test" } },
      {
        ...
        body: { performedAt: now, exercise: "Squat" },
      },
      {
        name: "/api/progress-photos",
      ...
      },
      {
        name: "/api/analyses",
        path: "/api/analyses",
        body: { planDate: now, contentMd: "Analysis" },
      },
      {
        name: "/api/progress-report",
        path: "/api/progress-report",
        body: { reportDate: now, totalSessions: 1 },
      },
    ];

    for (const t of tests) {
      try {
        const { res, body } = await doFetch(t.path, {
          method: "POST",
          headers,
